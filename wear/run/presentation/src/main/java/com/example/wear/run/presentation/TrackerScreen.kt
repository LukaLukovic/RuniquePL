...
fun TrackerScreenRoot(
    ...
    val isServiceRunning by ActiveRunService.isServiceRunning.collectAsStateWithLifecycle()
    LaunchedEffect(
        key1 = state.isRunActive,
        key2 = state.hasStartedRunning,
        key3 = isServiceRunning,
    ) {
        if (state.isRunActive && isServiceRunning.not()) {
...
private fun TrackerScreen(
    ...
    LaunchedEffect(key1 = true) {
        ...
        if (hasBodySensorsPermission.not())
            permissions.add(Manifest.permission.BODY_SENSORS)

        if (hasNotificationPermission.not() && Build.VERSION.SDK_INT >= 33)
    ...
    if (state.isConnectedPhoneNearby) {
        Column(
            modifier = Modifier
                ...
                .ambientMode(state.isAmbientModeActive, state.burnInProtectionRequired),
